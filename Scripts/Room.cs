...
public class Room : MonoBehaviour
{
    ...
    void Start()
    {
       /* DownDoor.SetActive(IsDownRoom);
        ...
        Gmap = GameObject.Find("MapPoint").GetComponent<GenerateMap>();
        SwitchRoomType();
    }
    void FightRoomSet()
    {
        GoblinClon("1(0.5,0.5)");
    }
    void SwitchRoomType()
    {
        if (Step > 2)
    ...
    private void Update()
    {
        if(Physics2D.OverlapArea(transform.position - new Vector3(10, 7), transform.position + new Vector3(10, 7), PlayerLayer))
        {
            Gmap.PlayerPlace = RoomXuhao;
        }
    }
   void GoblinClon(string Code)
    {
        //Code rule: 第一项表示项目数，之后的表示Vector3类的pos  Ex: 2(1,2)(3,4)
        for (int i = 0; i < Code[0]-48; i++)
        {
            Instantiate(Monsters.M_Goblin,ParseCodePos(Code,i)+transform.position, Quaternion.identity);
        }
    }
