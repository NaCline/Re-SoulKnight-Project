...
public class MonsterMove : MonoBehaviour
{
    ...
    //打印最小路径信息
   /* int GetClosestPoint(Vector3 Player)
    {
        float min = 11454;
        int PlayerPos = -1;
        for (int i = 0; i < MAXVEX; i++)
        {
            if (min > (Player - pos[i]).magnitude)
            {
                min = (Player - pos[i]).magnitude;
                PlayerPos = i;
            }
        }
        return PlayerPos;
    ...
    public IEnumerator TouchBullet()
    {
        ...
        float x = Mathf.Cos(direction / Mathf.Rad2Deg) * r/14, y = Mathf.Sin(direction / Mathf.Rad2Deg) * r/14;
        for (int i = 0; i < 14; i++)
        {
            transform.eulerAngles = new Vector3(0, 0, direction);
